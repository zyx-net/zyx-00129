...
- **审计包（Audit Package）**：
  ...
  - **导入预检（Precheck）**：
    - 单独的预检入口，不落库即可查看所有导入风险
    - 一次性检测：会话重名、冲突分支（reject/overwrite/auto-rename）、配置漂移明细、版本兼容性、重复导入来源
    - 终端友好的结构化报告输出，适合人直接阅读
    - 预检结果持久化保存（`.irec_prechecks/`），跨重启可查
    - 正式导入后，预检结论和最终处理方式写入 history 和 audit info
- **关账管理（Closeout）**：
  - 核对完成后一键封账，正式确认结账状态
  - **关账前置检查**：自动检测未匹配项、挂起项、已撤销未复核记录，默认阻止直接关账
  - **强制关账**：支持 `--force` 参数强制关账，需提供明确原因
  - **关账记录**：独立存储关账人、时间、备注、当时的发票/流水/匹配完整汇总快照
  - **关账后限制**：禁止导入、自动匹配、人工匹配、挂起、撤销等修改操作
  - **解账功能**：显式解账需填写解账人及原因，解账后恢复操作权限
  - **完整审计链**：关账/解账全部写入操作历史，status 面板实时显示关账状态
  - **快照/审计包兼容**：关账记录随快照、审计包完整导出导入，状态和摘要不串
  - **同名恢复兼容**：overwrite 或 auto-rename 时关账状态和摘要正确隔离
...
irec audit precheck <审计包文件> --as <目标会话名> --reject      # 导入预检（不落库，先看风险）
irec audit precheck <审计包文件> --as <目标会话名> --overwrite --apply-config  # 预检+指定参数
irec audit precheck-list                                          # 列出已保存的预检记录（跨重启可查）
irec audit precheck-show <预检ID>                                # 查看指定预检结果的详细信息
irec audit precheck-clear --yes                                  # 清空所有已保存的预检结果

# 关账管理
irec close check                          # 关账前置检查（未匹配/挂起/撤销未复核）
irec close do --by 财务-张三 --notes "2026年5月月末结账"  # 执行关账
irec close do --by 财务-张三 --notes "月末结账" --force --force-reason "仍有未匹配项，确认关账"  # 强制关账
irec close list                           # 列出所有关账记录
irec close show <close_id>                 # 查看指定关账记录详情（含汇总快照）
irec close export -o close_summary.json    # 导出关账摘要为JSON
irec unclose --by 财务-李四 --reason "发现漏记一笔，需要补充"  # 解账，需填写原因
```
...
# 3.1 基本预检 - 无冲突场景
irec audit precheck may_2026_final.irecaudit --as "audit_precheck_demo" --reject
# => 输出结构化的导入预检报告：
#   ═══════════════════════════════════════════════════════════
#     导入预检报告
#   ═══════════════════════════════════════════════════════════
#     预检ID        : precheck_xxxxxxxxxxxx
#     预检时间      : 2026-xx-xxTxx:xx:xx
#     审计包文件    : may_2026_final.irecaudit
#     审计包ID      : audit_xxxxxxxxxxxx
#     目标会话名    : audit_precheck_demo
#     冲突模式      : reject
#     应用配置      : 否
#   ───────────────────────────────────────────────────────────
#     预检结论      : ✓ 可以导入
...
#       来源文件    : 2
#   ═══════════════════════════════════════════════════════════
#     执行导入: irec audit import "may_2026_final.irecaudit" --as "audit_precheck_demo"
#   ═══════════════════════════════════════════════════════════

# 验证：预检不落库，目标会话不会被创建
irec status -s audit_precheck_demo
# => 错误：会话不存在（预检仅评估，不写入）

# 3.2 预检 - 配置漂移场景
irec config set days_tol 10
irec audit precheck may_2026_final.irecaudit --as "audit_drift_check" --overwrite --apply-config
# => 预检报告中：
#   配置检查
#     配置完整性  : ✓ 完整
#     配置漂移    : ⚠ 1 项漂移
#       - date_tolerance_days: 审计包=3, 当前=10
#   底部导入命令提示包含 --apply-config
irec config set days_tol 3

# 3.3 预检 - 冲突场景（目标会话已存在）
irec session create audit_conflict_precheck

irec audit precheck may_2026_final.irecaudit --as "audit_conflict_precheck" --reject
# => 预检结论: ✗ 无法导入
#   会话冲突分析
#     目标会话已存在: 是
#     处理方式: 拒绝导入（冲突模式 reject）
#     建议: 使用 --overwrite 覆盖或 --auto-rename 另存新副本

irec audit precheck may_2026_final.irecaudit --as "audit_conflict_precheck" --auto-rename
# => 预检结论: ✓ 可以导入（将重命名）
#   会话冲突分析
#     目标会话已存在: 是
#     处理方式: 自动重命名（另存新副本）
#     最终会话名: audit_conflict_precheck_restored

irec audit precheck may_2026_final.irecaudit --as "audit_conflict_precheck" --overwrite
# => 预检结论: ✓ 可以导入（将覆盖）
#   会话冲突分析
#     目标会话已存在: 是
#     处理方式: 覆盖已存在的会话
#     最终会话名: audit_conflict_precheck

# 3.4 预检 - 重复导入来源检查
irec audit precheck may_2026_final.irecaudit --as "default" --overwrite -s default
# => 重复导入来源检查
#     重复来源    : ⚠ 发现 2 个重复导入来源
#     仅审计包    : 0 个文件
#     仅目标会话  : 0 个文件
#   警告: 发现 2 个重复导入来源，若两边独立做过匹配，数据可能不一致

# 3.5 列出与查看预检记录（跨重启可查）
irec audit precheck-list
# => 列出所有已保存的预检记录：
#   预检记录存储目录: .irec_prechecks
#   预检ID                   审计包                          预检时间                 目标会话               可导入
#   --------------------------------------------------------------------------------------------------------------
#   precheck_xxxxxxxxxxxx  may_2026_final.irecaudit       2026-xx-xxTxx:xx:xx  audit_precheck_demo    ✓

irec audit precheck-show precheck_xxxxxxxxxxxx
# => 显示与 precheck 命令输出完全相同的完整预检报告
#    （同一份摘要结论，重启CLI后仍可查看）
...
#    所有合计金额与导出前完全一致
#    预检ID: precheck_xxxxxxxxxxxx（与导入前预检的结论一致）
...
#    apply_config、config_drift_detected、duplicate_source_files、
#    precheck_id、precheck_summary 等详情

# 8.5 导入后复查 - 用 precheck-show 查看同一份预检结论
irec audit precheck-show precheck_xxxxxxxxxxxx
# => 显示与导入前完全相同的预检报告
#    （同一份摘要结论，可用于事后排查和交接审计）
...
# 9.1 预检记录跨重启一致性
irec audit precheck-list
# => 重启后仍能看到所有预检记录（保存在 .irec_prechecks/）
#    同一份预检结论，CLI 重新打开后依然可查
...
irec session delete audit_conflict_precheck --yes

# 清理预检记录（可选）
irec audit precheck-clear --yes
```
...
# 此时尝试直接关账会被拒绝
irec close do --by 财务-李明 --notes "2026年5月月末结账"
...
# 确认待处理项无误后，使用 --force 强制关账
irec close do --by 财务-李明 --notes "2026年5月月末结账" \
              --force --force-reason "未匹配项为跨月差异，确认关账"
# => 输出：
#   ═══════════════════════════════════════════════════════════
#     ✅ 已关账  会话 'default' 已正式封账
#   ═══════════════════════════════════════════════════════════
#     关账ID      : close_xxxxxxxxxxxx
#     关账时间    : 2026-06-22Txx:xx:xx
#     关账人      : 财务-李明
#     关账备注    : 2026年5月月末结账
#     关账类型    : 强制关账
#     强制原因    : 未匹配项为跨月差异，确认关账
#
#     关账时汇总:
#       发票: 总 8  已匹配 6  未匹配 2  挂起 0
#       流水: 总 10  已匹配 7  未匹配 3  挂起 0
#       匹配: 活动 6  已撤销 0
#   ═══════════════════════════════════════════════════════════
#     【关账后限制】 禁止: 导入 / 自动匹配 / 人工匹配 / 挂起 / 撤销
#                    如需修改，请先执行: irec unclose --by <操作人> --reason <原因>
#   ═══════════════════════════════════════════════════════════

# 查看 status，显示已关账状态
irec status
# => 状态面板顶部显示：
#   【✅ 已关账】 关账人: 财务-李明  关账时间: 2026-06-22Txx:xx:xx
#           备注: 2026年5月月末结账
#   ...
#   ─────────────────────────────────────────────────────────
#   【关账后限制】 禁止: 导入 / 自动匹配 / 人工匹配 / 挂起 / 撤销
...
# 尝试导入发票 - 应被拒绝
irec imp invoice samples/invoices_good.csv
# => Error: 会话已关账，禁止执行『导入发票』操作。如需继续，请先执行 `irec unclose` 解账并说明原因。

# 尝试自动匹配 - 应被拒绝
irec match
# => Error: 会话已关账，禁止执行『自动匹配』操作。...

# 查看关账记录列表
irec close list
# => 输出：
#   ═══════════════════════════════════════════════════════════
#     关账记录  会话: default  (当前状态: 已关账)
#   ═══════════════════════════════════════════════════════════
#   关账ID             状态        关账时间                   关账人         备注
#   --------------------------------------------------------------------------------------------------------------
#   close_xxxxxxxxxx ✅ 已关账     2026-06-22Txx:xx:xx    财务-李明        2026年5月月末结账
#   ═══════════════════════════════════════════════════════════
#     查看详情: irec close show <关账ID>
#     导出摘要: irec close export -o <输出路径>
#   ═══════════════════════════════════════════════════════════

# 查看关账记录详情（含完整汇总快照）
irec close show close_xxxxxxxxxx
# => 输出：
#   ═══════════════════════════════════════════════════════════
#     关账记录详情
#   ═══════════════════════════════════════════════════════════
#     关账ID      : close_xxxxxxxxxx
#     关账时间    : 2026-06-22Txx:xx:xx
#     关账人      : 财务-李明
#     关账备注    : 2026年5月月末结账
#
#     关账时汇总:
#       发票: 总 8  已匹配 6  未匹配 2  挂起 0
#             金额 总 100,151.25  已匹配 72,051.25
#       流水: 总 10  已匹配 7  未匹配 3  挂起 0
#             金额 总 110,951.25  已匹配 72,051.25
#       匹配: 活动 6  已撤销 0
#       历史操作: N 条
#       已导入文件: 2 个
#
#     关账时未匹配明细:
#       未匹配发票 (2 张):
#         - INV-2026-005 百度在线 18,900.00 2026-06-12
#         - INV-2026-008 小米科技 9,200.00 2026-06-20
#       未匹配流水 (3 笔):
#         - TXN20260612005 百度 9,400.00 2026-06-13
#         - TXN20260620009 小米 9,500.00 2026-06-21
#         - TXN20260622010 网易 20,000.00 2026-06-22
#   ═══════════════════════════════════════════════════════════
...
irec close export -o may_2026_close_summary.json
# => [OK] 关账摘要已导出 -> D:\...\may_2026_close_summary.json
#       包含关账记录: 1 条

# 导出的 JSON 包含完整的关账信息和汇总快照，可用于审计和归档
...
# 导出已关账会话的快照
irec snapshot export -o may_2026_closed --notes "关账后快照"
# => 快照包含完整的关账记录和状态

# 导入为新会话
irec snapshot import may_2026_closed.irecsnap --as may_2026_closed_restored --switch
# => 新会话状态：
#   【✅ 已关账】 关账人: 财务-李明  关账时间: 2026-06-22Txx:xx:xx
#           备注: 2026年5月月末结账
#   ...（关账记录、汇总快照、限制提示完全一致）
...
# 验证关账后限制在恢复后仍生效
irec imp invoice samples/invoices_good.csv --session may_2026_closed_restored
# => Error: 会话已关账，禁止执行『导入发票』操作。...
...
# 解账需填写解账人及原因
irec unclose --by 财务-王芳 --reason "发现漏记一笔发票，需要补充录入"
# => 输出：
#   ═══════════════════════════════════════════════════════════
#     🔓 已解账  会话 'default' 已解除关账状态
#   ═══════════════════════════════════════════════════════════
#     原关账ID    : close_xxxxxxxxxx
#     原关账时间  : 2026-06-22Txx:xx:xx
#     原关账人    : 财务-李明
#     原关账备注  : 2026年5月月末结账
#
#     解账时间    : 2026-06-22Txx:xx:xx
#     解账人      : 财务-王芳
#     解账原因    : 发现漏记一笔发票，需要补充录入
#
#     【提示】 会话现已恢复可编辑状态
#             修改完成后请再次执行 `irec close do` 重新关账
#   ═══════════════════════════════════════════════════════════
...
# 补充数据完成后，再次关账
irec close do --by 财务-王芳 --notes "补充数据后重新关账" \
              --force --force-reason "仍有未匹配项，确认关账"
# => 成功关账，此时关账记录列表中有 2 条记录
...
#   关账ID             状态        关账时间                   关账人         备注
#   --------------------------------------------------------------------------------------------------------------
#   close_xxxxxxxxxx 🔓 已解账     2026-06-22Txx:xx:xx    财务-李明        2026年5月月末结账
#   close_yyyyyyyyyy ✅ 已关账     2026-06-22Txx:xx:xx    财务-王芳        补充数据后重新关账

# ======================
# 9. 历史记录 - 关账/解账完整审计链
# ======================

irec show history --action close_session
# => 显示所有关账操作，每条包含：
#   [2026-06-22 xx:xx:xx] close_session
#     ┌─────────────────────────────────────────────────┐
#     │ 关账ID    : close_xxxxxxxxxx                     │
#     │ 关账人    : 财务-李明                             │
#     │ 备注      : 2026年5月月末结账                      │
...
irec show history --action unclose_session
# => 显示所有解账操作，每条包含：
#   [2026-06-22 xx:xx:xx] unclose_session
#     ┌─────────────────────────────────────────────────┐
#     │ 原关账ID  : close_xxxxxxxxxx                     │
#     │ 解账人    : 财务-王芳                             │
#     │ 解账原因  : 发现漏记一笔发票，需要补充录入          │
...
# ======================
# 10. 同名恢复 - overwrite / auto-rename 状态不串
# ======================

# 创建另一个会话，不关账
irec session create close_test_other
irec session switch close_test_other
irec status
# => 【🔓 未关账】

# 用关账后的会话快照 overwrite 这个会话
irec snapshot import may_2026_closed.irecsnap --as close_test_other --overwrite
# => 【✅ 已关账】（状态来自导入的快照，与原会话隔离）

# 验证 overwrite 后的关账状态正确
irec close list --session close_test_other
# => 关账记录与原会话完全一致

# 用 auto-rename 导入同一份快照
irec snapshot import may_2026_closed.irecsnap --as close_test_other --auto-rename
# => 自动重命名为 close_test_other_restored

# 验证新会话关账状态正确，与其他会话隔离
irec status --session close_test_other_restored
# => 【✅ 已关账】 关账人: 财务-李明 ...
irec status --session close_test_other
# => 【✅ 已关账】 关账人: 财务-李明 ...
irec status --session default
# => 【✅ 已关账】 关账人: 财务-王芳 ...（三个会话状态独立，互不干扰）

# 清理测试会话
irec session switch default
irec session delete may_2026_closed_restored --yes
irec session delete close_test_other --yes
irec session delete close_test_other_restored --yes
```
...
#   [H] 导入预检：不落库检测 / 三分支冲突 / 配置漂移 / 重复来源 / 持久化 / 跨重启 / 导入后复查

# 关账功能回归（A未关账正常操作 / B关账后限制 / C解账恢复 / D跨会话隔离 / E历史记录 / F快照审计包兼容）
python _test_closeout_regression.py
# => [PASSED] 所有关账回归测试通过，exit 0
# 覆盖子断言：
#   [A] 未关账会话的正常操作不受影响（导入/匹配/状态/历史）
#   [B] 关账后操作限制正确生效（导入/匹配均被拒绝，提示清晰）
#   [C] 解账后操作恢复正常（可重新导入/匹配）
#   [D] 关账不影响其他会话（多会话状态独立）
#   [E] 历史记录正确记录关账/解账动作（含操作人、原因、汇总快照）
#   [F] 现有快照、审计包功能不受关账影响（关账记录完整导出导入）

# 关账功能验收测试（跨重启/快照审计包导入导出/overwrite&auto-rename状态不串/完整操作链）
python _test_closeout_acceptance.py
# => 所有验收场景通过，exit 0
# 覆盖子断言：
#   跨重启后封账状态不丢
#   快照导出再导入后关账记录仍能对齐
#   审计包导出再导入后关账记录仍能对齐
#   同名恢复走 overwrite 时状态和摘要不串
#   同名恢复走 auto-rename 时状态和摘要不串
#   关账限制、解账、重新关账完整链路
```
...
| **关账前置检查** | `irec close check` | 自动检测未匹配发票/未匹配流水/挂起项/撤销未复核项，输出结构化报告，存在待处理项时阻止直接关账 |
| **关账-正常关账** | `irec close do --by 财务-张三 --notes "月末结账"` | 检查通过时成功关账，生成独立 CloseRecord，包含关账人/时间/备注/完整汇总快照 |
| **关账-强制关账** | `irec close do --by 财务-张三 --notes "月末结账" --force --force-reason "未匹配项为跨月差异"` | 存在待处理项时仍可强制关账，需提供明确原因，force 标记写入汇总快照和历史记录 |
| **关账后限制-导入** | 关账后执行 `irec imp invoice ...` | 明确拒绝，提示"会话已关账，禁止执行『导入发票』操作。如需继续，请先执行 `irec unclose` 解账并说明原因。" |
| **关账后限制-自动匹配** | 关账后执行 `irec match` | 明确拒绝，提示信息包含操作名称和解账指引 |
| **关账后限制-人工匹配** | 关账后执行 `irec manual match ...` | 明确拒绝，提示信息包含操作名称和解账指引 |
| **关账后限制-挂起** | 关账后执行 `irec manual suspend-inv ...` | 明确拒绝，提示信息包含操作名称和解账指引 |
| **关账后限制-撤销** | 关账后执行 `irec manual reverse ...` | 明确拒绝，提示信息包含操作名称和解账指引 |
| **关账状态显示** | 关账后执行 `irec status` | 状态面板顶部显示【✅ 已关账】，包含关账人、关账时间、备注；底部显示【关账后限制】提示和解账命令指引 |
| **解账功能** | `irec unclose --by 财务-李四 --reason "发现漏记一笔，需要补充"` | 成功解除关账状态，标记原 CloseRecord 为 is_unclosed，写入解账人、解账时间、解账原因 |
| **解账后状态显示** | 解账后执行 `irec status` | 状态面板顶部显示【🔓 未关账】历史关账 N 次（当前已解账），历史关账次数清晰可见 |
| **解账后操作恢复** | 解账后执行 `irec imp invoice ...` / `irec match` | 操作恢复正常，不再被拦截 |
| **关账记录列表** | `irec close list` | 列出所有关账记录，包含关账ID、状态（已关账/已解账）、关账时间、关账人、备注 |
| **关账记录详情** | `irec close show <close_id>` | 显示完整关账详情：基本信息 + 关账时汇总（发票/流水/匹配统计）+ 未匹配明细 |
| **关账摘要导出** | `irec close export -o summary.json` | 导出 JSON 格式的关账摘要，包含所有关账记录、当前关账状态、完整汇总快照 |
| **跨重启关账状态不丢** | 关账后重启 CLI 执行 `irec status` | is_closed 状态、close_records 列表、关账人/时间/备注均保持不变 |
| **快照导出-关账记录** | 关账后执行 `irec snapshot export ...` | 快照包含完整的 is_closed 状态和 close_records 列表，汇总快照完整 |
| **快照导入-关账记录对齐** | 导入包含关账记录的快照 | 新会话 is_closed=True，close_records 与原会话完全一致，关账人/时间/备注/汇总快照全部对齐 |
| **审计包导出-关账记录** | 关账后执行 `irec audit export ...` | 审计包包含完整的 is_closed 状态和 close_records 列表 |
| **审计包导入-关账记录对齐** | 导入包含关账记录的审计包 | 新会话 is_closed=True，close_records 与原会话完全一致，所有字段对齐 |
| **同名恢复-overwrite 状态不串** | 快照导入使用 `--overwrite` 覆盖已存在会话 | 目标会话被完整覆盖，is_closed 状态和 close_records 来自导入的快照，与原会话隔离 |
| **同名恢复-auto-rename 状态不串** | 快照导入使用 `--auto-rename` 处理重名 | 自动生成新会话名，新会话 is_closed 状态和 close_records 正确，原会话保持不变 |
| **历史记录-关账** | `irec show history --action close_session` | 每条关账记录包含：关账ID、关账人、备注、关账类型（正常/强制）、强制原因、汇总快照 |
| **历史记录-解账** | `irec show history --action unclose_session` | 每条解账记录包含：原关账ID、解账人、解账原因、原关账人、原关账备注 |
| **关账后限制-导入后恢复** | 快照/审计包导入后执行 `irec imp invoice ...` | 关账限制继续生效，被正确拦截 |
| **一键关账回归脚本** | `python _test_closeout_regression.py` | 输出 `[PASSED]` + exit 0，覆盖未关账正常操作/关账后限制/解账恢复/跨会话隔离/历史记录/快照审计包兼容 六大类 |
| **一键关账验收脚本** | `python _test_closeout_acceptance.py` | 所有验收场景通过 + exit 0，覆盖跨重启/快照导入导出/审计包导入导出/overwrite状态不串/auto-rename状态不串/完整操作链 |
...
**Q: 关账后发现需要修改数据怎么办？**
A: 使用 `irec unclose --by <操作人> --reason <原因>` 解账，修改完成后重新执行 `irec close do` 关账。所有操作都会记入历史，便于审计追溯。

**Q: 关账检查提示有未匹配项可以关账吗？**
A: 默认不可以。如果确认未匹配项无需处理（如跨月差异），可以使用 `--force` 参数强制关账，但必须提供 `--force-reason` 说明原因，强制关账标记会永久保存在关账记录中。

**Q: 关账记录可以删除吗？**
A: 不可以。关账记录是审计痕迹的一部分，一旦生成不可删除。解账只是标记原关账记录为"已解账"，原记录仍然保留，便于追溯。

**Q: 快照/审计包导入后，关账状态会保留吗？**
A: 会的。关账状态、关账记录、汇总快照都会随快照/审计包完整导出和导入，导入后的会话与原会话关账状态完全一致，关账后限制也继续生效。
