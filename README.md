...
以下命令链覆盖所有验收要点：**自动匹配 1 笔 + 人工匹配 1 笔 + 非法行报错 + 重复导入拦截 + 一对多超出容差保持未解决 + 会话重启持久化验证**。
...
| 会话重启持久化 | `irec status` 重启后对照 | session_id、合计、未处理、撤销记录均不变 |
| 报告导出 | `irec report` | JSON + 5 张 CSV 生成 |
