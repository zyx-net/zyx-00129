...
assert_true("目标" in out and SESS_CONFLICT in out and "已存在" in out,
            "C2: --auto-rename 模式下显示目标会话原名+已存在提示")
assert_true("另存新副本" in out, "C2: --auto-rename 模式下显示另存新副本提示")
assert_true(f"{SESS_CONFLICT}_restored" in out,
            "C2: 重命名后的会话名出现在输出中")
assert_true("目标会话(原定)" in out and SESS_CONFLICT in out,
            "C2: 输出中明确展示『目标会话(原定)』字段并填入原定名 audit_conflict")
...
# C2b: 验证 auto-rename 场景下历史记录 conflict_branch_result 中 original_name != final_name
import_hist_c2 = [h for h in sess_c2.history if h.action == "audit_import"][-1]
c2_conflict = import_hist_c2.details.get("conflict_branch_result", {})
assert_true(c2_conflict.get("original_name") == SESS_CONFLICT,
            f"C2b: 历史记录 conflict_branch_result.original_name = {SESS_CONFLICT} （原定目标名）")
assert_true(c2_conflict.get("final_name") == renamed_names[0],
            f"C2b: 历史记录 conflict_branch_result.final_name = {renamed_names[0]} （重命名后）")
assert_true(c2_conflict.get("original_name") != c2_conflict.get("final_name"),
            "C2b: auto-rename 场景下 original_name ≠ final_name（不再是『新名→新名』）")
assert_true(c2_conflict.get("intended_session_name") == SESS_CONFLICT,
            "C2b: intended_session_name 记录了用户原定的目标会话名")
assert_true(c2_conflict.get("renamed") is True,
            "C2b: renamed=True 标记正确")
# C2c: 验证 target_session_name 是原定名，final_session_name 是最终名
c2_details = import_hist_c2.details
assert_true(c2_details.get("target_session_name") == SESS_CONFLICT,
            f"C2c: 历史详情 target_session_name = {SESS_CONFLICT} （用户原定名）")
assert_true(c2_details.get("final_session_name") == renamed_names[0],
            f"C2c: 历史详情 final_session_name = {renamed_names[0]} （解析后最终名）")
...
# ============================================================
# 组 M：auto-rename 三处复查入口完全对齐
# 要求：audit info / precheck-show / show history 三处展示的原名→新名、
#       最终处理方式、来源审计包、配置漂移摘要完全一致
# ============================================================
print("\n--- 组 M：auto-rename 三处复查入口完全对齐 ---")

# M1: 对 L1 中的 auto-rename 导入做三处入口复查
# 先拿到会话历史中的原始详情
sess_m_rename = sm_l3.load(actual_rename_sess)
hist_m_rename = None
for h in sess_m_rename.history:
    if h.action == "audit_import" and h.details.get("precheck_id") == rename_precheck_id:
        hist_m_rename = h
        break
assert_true(hist_m_rename is not None, "M1: 找到 auto-rename 导入历史记录")
m_details = hist_m_rename.details
m_conflict = m_details.get("conflict_branch_result", {})
m_intended = (
    m_conflict.get("intended_session_name")
    or m_conflict.get("original_name")
    or m_details.get("target_session_name")
)
m_final = m_conflict.get("final_name") or m_details.get("final_session_name") or actual_rename_sess
assert_true(m_intended == "rename_sess_base",
            f"M1: 历史记录中的原定目标名 = rename_sess_base（实际: {m_intended!r}）")
assert_true(m_final.startswith("rename_sess_base_restored"),
            f"M1: 历史记录中的最终会话名 = rename_sess_base_restored*（实际: {m_final!r}）")
assert_true(m_intended != m_final,
            f"M1: 原定名≠最终名（{m_intended!r} ≠ {m_final!r}），不再是『新名→新名』")

# M2: 入口 A - audit info（反向查找）
res, out = run(["audit", "info", "audit_A_full.irecaudit"])
# 检查 audit info 中的 auto-rename 条目是否正确展示 rename_sess_base → rename_sess_base_restored*
assert_true("rename_sess_base" in out and "restored" in out,
            "M2: audit info 复查中同时出现原定名和最终名")
assert_true(rename_precheck_id in out,
            "M2: audit info 复查中包含 auto-rename 的预检ID")
assert_true("自动重命名" in out or "auto-rename" in out.lower() or "另存新副本" in out,
            "M2: audit info 复查中标记处理方式为 auto-rename/另存新副本")
# 检查是否出现"目标 'XXX' 被占用"之类的描述（能看出原定名）
assert_true("rename_sess_base" in out and ("被占用" in out or "另存新副本" in out or "→" in out),
            "M2: audit info 冲突分支中能看出归档原定往哪个会话落")

# M3: 入口 B - precheck-show（预检回写）
res, out = run(["audit", "precheck-show", rename_precheck_id])
assert_true("rename_sess_base" in out,
            "M3: precheck-show 中出现目标会话原名 rename_sess_base")
assert_true(actual_rename_sess in out,
            f"M3: precheck-show 中出现最终导入会话名 {actual_rename_sess}")
assert_true("自动重命名" in out or "auto-rename" in out.lower() or "另存" in out,
            "M3: precheck-show 最终处理方式标记为 auto-rename")
# 检查导入回写部分的冲突分支描述
assert_true("目标" in out and "rename_sess_base" in out and ("被占用" in out or "→" in out or "另存" in out),
            "M3: precheck-show 实际导入结果冲突分支能看出原定名")
assert_true("配置漂移摘要" in out,
            "M3: precheck-show 中包含配置漂移摘要")

# M4: 入口 C - show history（专用格式化）
res, out = run(["show", "history",
                "--action", "audit_import", "--session", actual_rename_sess])
assert_true("预检结论" in out, "M4: history 专用格式化框展示预检结论")
assert_true(rename_precheck_id in out, "M4: history 中包含预检ID")
assert_true("自动重命名" in out or "另存新副本" in out,
            "M4: history 最终处理方式 = auto-rename / 另存新副本")
# 冲突分支结果能看出原名→新名
assert_true("rename_sess_base" in out,
            "M4: history 冲突分支展示原定目标名")
assert_true(actual_rename_sess in out or "restored" in out,
            "M4: history 冲突分支展示最终会话名")
# 目标会话行能看出原定→最终的转换
assert_true("原定" in out and "→" in out and "最终" in out,
            "M4: history 目标会话行明确展示『原定 X → 最终 Y』")
assert_true("配置漂移摘要" in out,
            "M4: history 包含配置漂移摘要")
assert_true("来源审计包" in out and "audit_A_full" in out,
            "M4: history 包含来源审计包信息")
assert_true("其他入口复查" in out,
            "M4: history 包含其他入口复查跳转建议")

# M5: 三处入口的最终处理方式 & 预检ID & 原定名 对齐
res_m_info, out_m_info = run(["audit", "info", "audit_A_full.irecaudit"])
res_m_pc, out_m_pc = run(["audit", "precheck-show", rename_precheck_id])
res_m_hist, out_m_hist = run(["show", "history",
                              "--action", "audit_import",
                              "--session", actual_rename_sess])
# 预检ID一致
assert_true(rename_precheck_id in out_m_info, "M5: audit info 预检ID = L1 的预检ID")
assert_true(rename_precheck_id in out_m_hist, "M5: show history 预检ID = L1 的预检ID")
# 都出现原定目标名 rename_sess_base
assert_true("rename_sess_base" in out_m_info, "M5: audit info 含原定名")
assert_true("rename_sess_base" in out_m_pc, "M5: precheck-show 含原定名")
assert_true("rename_sess_base" in out_m_hist, "M5: show history 含原定名")
# 都出现最终会话名（或 restored 标记）
has_final_info = actual_rename_sess in out_m_info or ("rename_sess_base" in out_m_info and "restored" in out_m_info)
has_final_pc = actual_rename_sess in out_m_pc or ("rename_sess_base" in out_m_pc and "restored" in out_m_pc)
has_final_hist = actual_rename_sess in out_m_hist or ("rename_sess_base" in out_m_hist and "restored" in out_m_hist)
assert_true(has_final_info, "M5: audit info 含最终会话名")
assert_true(has_final_pc, "M5: precheck-show 含最终会话名")
assert_true(has_final_hist, "M5: show history 含最终会话名")
# 都标记处理方式为 auto-rename（或"自动重命名"、"另存新副本"）
rename_tags = ["自动重命名", "auto-rename", "另存新副本", "另存"]
assert_true(any(t in out_m_info for t in rename_tags),
            "M5: audit info 标记处理方式=auto-rename")
assert_true(any(t in out_m_pc for t in rename_tags),
            "M5: precheck-show 标记处理方式=auto-rename")
assert_true(any(t in out_m_hist for t in rename_tags),
            "M5: show history 标记处理方式=auto-rename")
# 都有配置漂移摘要标记
assert_true("配置漂移" in out_m_info or "漂移" in out_m_info,
            "M5: audit info 含配置漂移信息")
assert_true("配置漂移" in out_m_pc or "漂移" in out_m_pc,
            "M5: precheck-show 含配置漂移信息")
assert_true("配置漂移" in out_m_hist or "漂移" in out_m_hist,
            "M5: show history 含配置漂移信息")

# M6: overwrite 场景对照检查（确保 overwrite 也展示覆盖了哪个原定名）
# overwrite 的 import 是在 I0 组做的 review_target_sess
res, out = run(["show", "history",
                "--action", "audit_import", "--session", "review_target_sess"])
assert_true("覆盖" in out or "overwrite" in out.lower(),
            "M6: overwrite 场景 history 标记为覆盖")
assert_true("review_target_sess" in out,
            "M6: overwrite 场景能看出覆盖的目标会话名")
# 冲突分支: 覆盖目标会话 'review_target_sess'
assert_true("review_target_sess" in out and ("覆盖" in out or "替换" in out),
            "M6: overwrite 场景冲突分支描述含被覆盖会话名")


# ============================================================
# 组 N：跨重启复查（模拟重新加载后所有字段仍对齐）
# 要求：重新加载 SessionManager / PrecheckStore 后，三处入口信息不变
# ============================================================
print("\n--- 组 N：跨重启复查一致性 ---")

# N1: 先记录重启前 auto-rename 场景三处入口的关键字段值
# 从持久化文件直接读
with open(os.path.join(".irec_prechecks", f"{rename_precheck_id}.json"), "r", encoding="utf-8") as f:
    pc_before_restart = json.load(f)
with open(os.path.join(".irec_sessions", f"{actual_rename_sess}.json"), "r", encoding="utf-8") as f:
    sess_before_restart = json.load(f)
# 找 audit_import 历史
hist_before = None
for h in sess_before_restart["history"]:
    if h["action"] == "audit_import" and h["details"].get("precheck_id") == rename_precheck_id:
        hist_before = h
        break
assert_true(hist_before is not None, "N1: 从磁盘读取到导入前历史")
hd_before = hist_before["details"]
cbr_before = hd_before.get("conflict_branch_result", {})

# N2: 模拟重启 - 删除所有 Python 引用、GC 回收后重新 new
del sm_l3, cfg_l3, sess_m_rename, sess_overwrite, sess_rename
import gc
gc.collect()

# 重新创建所有管理器
from invoice_reconcile.config import Config as _CfgN
from invoice_reconcile.session import SessionManager as _SmN
from invoice_reconcile.audit import PrecheckStore as _PsN, AuditManager as _AmN
cfg_n = _CfgN.load()
sm_n = _SmN(cfg_n.session_dir)
ps_n = _PsN()
am_n = _AmN()

# N3: 预检跨重启 - 7 个回写字段不丢失
pc_reloaded = ps_n.load(rename_precheck_id)
assert_true(pc_reloaded is not None, "N3: 重启后能加载 auto-rename 的预检")
for fk in ["import_executed", "imported_at", "imported_session_name",
           "imported_session_id", "actual_final_action",
           "actual_conflict_mode", "config_drift_summary"]:
    assert_true(getattr(pc_reloaded, fk, None) is not None and getattr(pc_reloaded, fk) != "",
                f"N3: 重启后预检字段 {fk} 不丢失")
assert_true(pc_reloaded.import_executed is True, "N3: 重启后 import_executed=True")
assert_true(pc_reloaded.actual_final_action == "rename",
            "N3: 重启后 actual_final_action=rename")
assert_true(pc_reloaded.imported_session_name == actual_rename_sess,
            f"N3: 重启后 imported_session_name={actual_rename_sess} 不变")
assert_true(pc_reloaded.target_session_name == "rename_sess_base",
            "N3: 重启后 target_session_name（原定名）= rename_sess_base 不变")

# N4: 会话历史跨重启 - 新增字段（conflict_branch_result 等）不丢失
sess_n_reloaded = sm_n.load(actual_rename_sess)
hist_n_reloaded = None
for h in sess_n_reloaded.history:
    if h.action == "audit_import" and h.details.get("precheck_id") == rename_precheck_id:
        hist_n_reloaded = h
        break
assert_true(hist_n_reloaded is not None, "N4: 重启后会话历史仍存在")
hd_after = hist_n_reloaded.details
for fk in ["final_action", "final_action_reason", "conflict_branch_result",
           "config_drift_full", "import_timestamp", "precheck_summary",
           "target_session_name", "final_session_name"]:
    assert_true(fk in hd_after,
                f"N4: 重启后历史详情字段 {fk} 存在")
cbr_after = hd_after.get("conflict_branch_result", {})
# conflict_branch_result 的关键三项不丢失
assert_true("original_name" in cbr_after, "N4: conflict_branch_result.original_name 存在")
assert_true("final_name" in cbr_after, "N4: conflict_branch_result.final_name 存在")
assert_true("intended_session_name" in cbr_after,
            "N4: conflict_branch_result.intended_session_name 存在")
# 值不变
assert_true(cbr_after["original_name"] == "rename_sess_base",
            "N4: 重启后 conflict_branch_result.original_name = rename_sess_base")
assert_true(cbr_after["final_name"] == actual_rename_sess,
            f"N4: 重启后 conflict_branch_result.final_name = {actual_rename_sess} 不变")
assert_true(hd_after["target_session_name"] == "rename_sess_base",
            "N4: 重启后 target_session_name = rename_sess_base（用户原定名）")
assert_true(hd_after["final_session_name"] == actual_rename_sess,
            f"N4: 重启后 final_session_name = {actual_rename_sess} 不变")

# N5: 重启后再次调用 CLI 三处入口复查（验证完全一致）
# 入口 A: audit info
res_n1, out_n1 = run(["audit", "info", "audit_A_full.irecaudit"])
assert_true(rename_precheck_id in out_n1, "N5: 重启后 audit info 预检ID不变")
assert_true("rename_sess_base" in out_n1 and "restored" in out_n1,
            "N5: 重启后 audit info 原名→最终名不变")
# 入口 B: precheck-show
res_n2, out_n2 = run(["audit", "precheck-show", rename_precheck_id])
assert_true(rename_precheck_id in out_n2, "N5: 重启后 precheck-show 预检ID不变")
assert_true("rename_sess_base" in out_n2 and actual_rename_sess in out_n2,
            "N5: 重启后 precheck-show 原名→最终名不变")
# 入口 C: show history
res_n3, out_n3 = run(["show", "history",
                      "--action", "audit_import",
                      "--session", actual_rename_sess])
assert_true(rename_precheck_id in out_n3, "N5: 重启后 history 预检ID不变")
assert_true("原定" in out_n3 and "→" in out_n3 and "最终" in out_n3,
            "N5: 重启后 history 仍展示『原定→最终』")
assert_true("rename_sess_base" in out_n3,
            "N5: 重启后 history 冲突分支原名不变")

# N6: find_audit_imports 反向查找跨重启后字段完整
find_results = am_n.find_audit_imports(pc_before_restart["audit_id"], sm_n)
renamed_find = [r for r in find_results if r.get("precheck_id") == rename_precheck_id]
assert_true(len(renamed_find) == 1, "N6: 重启后 find_audit_imports 能找到 auto-rename 记录")
rf = renamed_find[0]
for fk in ["intended_session_name", "conflict_original_name",
           "conflict_final_name", "final_session_name",
           "target_session_name", "config_drift_full"]:
    assert_true(fk in rf and rf[fk] is not None,
                f"N6: find_audit_imports 返回包含 {fk}")
assert_true(rf["intended_session_name"] == "rename_sess_base",
            "N6: find_audit_imports intended_session_name 正确")
assert_true(rf["conflict_original_name"] == "rename_sess_base",
            "N6: find_audit_imports conflict_original_name 正确")
assert_true(rf["conflict_final_name"] == actual_rename_sess,
            f"N6: find_audit_imports conflict_final_name={actual_rename_sess} 正确")
...
print("  [M] auto-rename 三处复查对齐：原名→最终名/处理方式/漂移/来源 信息一致")
print("  [N] 跨重启复查：持久化字段完整、三处入口一致、find_audit_imports 不变")
print("=" * 70)
sys.exit(0)
